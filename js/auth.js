/* ============================================
   ...
   --------------------------------------------
   تسجيل الدخول:
   - البريد الإلكتروني + كلمة المرور فقط

   إنشاء الحساب:
   - الهاتف + PIN
   - أو البريد الإلكتروني + كلمة مرور

   - الجلسة تُحفظ محليًا وتُستعاد تلقائيًا
   - ترحيل تلقائي لبيانات النسخة المحلية القديمة
...
const Auth = (function () {
  ...
  function sb() {
    return (window.SupabaseConfig && SupabaseConfig.isReady())
  ...
  function clearLocalForOtherAccount(user, incomingPhone) {
    ...
    const legacySamePhone =
      String(cached.id || '').indexOf('t_') === 0 &&
      incomingPhone &&
      cached.phone === incomingPhone;
  ...
  function inferAuthMethod(user) {
    return SupabaseConfig.isPhoneAccountEmail(user && user.email)
      ? 'phone'
      : 'email';
  }
  ...
  function migrateLegacyIfNeeded(user, phone) {
    ...
    if (samePhone && (legacyId || cached.id !== user.id)) {
      ...
      UI.toast(
        'تم رفع بياناتك المحلية إلى حسابك السحابي',
        'success'
      );
    }
  }
  ...
  function restoreSession() {
    ...
    restoring = (async () => {
      ...
      const cached =
        Storage.get(Storage.KEYS.teacher, null);
      ...
      try {
        ...
        const session = await Promise.race([
          c.auth
            .getSession()
            .then(res =>
              (res.data && res.data.session) || null
            ),
        ...
        if (session && session.user) {
          ...
          const userPhone =
            ...
            (
              SupabaseConfig.isPhoneAccountEmail(user.email)
                ? String(user.email || '').split('@')[0]
                : ''
            );

          const cachedIsLegacy =
            !!(
              cached &&
              String(cached.id || '').indexOf('t_') === 0
            );
          ...
          if (cached && cached.id === user.id) {
            ...
            clearLocalForOtherAccount(
              user,
              userPhone
            );

            currentTeacher =
              await fetchProfileAsTeacher(user);
          ...
          if (currentTeacher.id !== user.id) {
            currentTeacher.id = user.id;
          }

          currentTeacher =
            withAuthInfo(currentTeacher, user);
  ...
  function init() {
    ...
    const t =
      Storage.get(Storage.KEYS.teacher);
  ...
  // ===== فحص رابط استعادة كلمة المرور =====

  function checkRecoveryLink() {

    const h =
      (
        window.__moallemyInitialHash ||
        location.hash ||
        ''
      );
  ...
  function isLogged() {
    return !!currentTeacher && !recoveryPending;
  }
  ...
  async function changeCredential(current, next) {
    ...
    if (!c) {
      return {
        ok: false,
        message: 'الخدمة السحابية غير متاحة حاليًا'
      };
    }
    ...
    try {
      ...
      if (error) {
        ...
        return {
          ok: false,
          message:
            SupabaseConfig.authErrorMessage(
              error,
              'email'
            )
        };
  ...
  function bindEvents() {
    ...
    // تبويبات تسجيل الدخول / إنشاء الحساب

    document
      .querySelectorAll('.auth-tab[data-tab]')
      .forEach(tab => {
    ...
    // لا يوجد مبدل لتسجيل الدخول
    // لأن الدخول بالبريد فقط
  ...
  function setLoginMethod(method) {
    ...
    // مهما كانت القيمة المرسلة
    // تسجيل الدخول = بريد فقط
    loginMethod = 'email';
  ...
  async function handleLogin(e) {
    ...
    if (!c) {
      ...
      UI.toast(
        'خدمة الدخول غير متاحة حاليًا — تحقق من الاتصال وأعد تحميل الصفحة',
        'error'
      );
    ...
    try {
      ...
      // Supabase Email Authentication فقط
      ...
      if (error) {
        ...
        // دعم الدخول من الكاش عند انقطاع الإنترنت
        const cached =
          Storage.get(
            Storage.KEYS.teacher,
            null
        ...
        if (
          ...
          UI.toast(
            'وضع عدم الاتصال: تم الدخول من الذاكرة المحلية',
            'info'
          );
      ...
      // إنشاء بيانات أساسية إذا لم يوجد Profile
  ...
  async function handleRegister(e) {
    ...
    if (!c) {
      ...
      UI.toast(
        'خدمة إنشاء الحسابات غير متاحة حاليًا — تحقق من الاتصال',
        'error'
      );
    ...
    try {
      ...
      const signupEmail =
        isEmail
          ? email
          : SupabaseConfig.phoneToEmail(phone);

      const signupPassword =
        isEmail
          ? pass
          : SupabaseConfig.pinToPassword(pin);
      ...
      if (!user || !data.session) {
        ...
        UI.toast(
          'تم إرسال رابط تأكيد إلى بريدك الإلكتروني — أكّده ثم سجّل الدخول',
          'info',
          5000
        );
  ...
  async function handleForgotPassword() {
    ...
    if (!email) {
      ...
      UI.toast(
        'أدخل بريدك الإلكتروني في الحقل أعلاه ثم اضغط "نسيت كلمة المرور"',
        'info',
        3500
      );
    ...
    if (!c) {
      ...
      UI.toast(
        'الخدمة غير متاحة حاليًا — تحقق من الاتصال',
        'error'
      );
    ...
    try {
      ...
      if (error) {
        ...
        UI.toast(
          'إذا كان البريد مسجلًا فستصلك رسالة بها رابط إعادة تعيين كلمة المرور — تحقق من صندوق الوارد وبريد الرسائل غير المرغوبة',
          'success',
          6000
        );
      }
  ...
  return {
    ...
    verifyPIN: verifyCredential,

    changePIN: changeCredential,
...
const DemoData = {
  ...
  load() {
    ...
    const teacher = {
      ...
      bio:
        'مدرس رياضيات للمرحلة الثانوية - خبرة 12 عامًا',
